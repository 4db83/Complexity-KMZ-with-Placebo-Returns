...
for ii = 1:length(seed_set)
  ...
  for demean = [0]
    ...
    for trnwin = trnwin_list
      ...
      file_list = KMZ_file_names_to_get(OUTPUT_DIR, trnwin, stdize_Y, demean, nSim);
      nFiles = length(file_list);
      fprintf("Getting: %.5g/%.5g.\n", [nFiles nSim])
      sep('*')
      % for ( ii = 1:nFiles )
      for ( ii = 1:nFiles )
...
% /mimer/NOBACKUP/groups/naiss2025-22-1033/complexity KMZ - placebo/_nsims_source_RFF_output_placebo_Ret_1017/maxP-12000-trnwin-12-gamma-2-stdize-1-demean-0-v2
