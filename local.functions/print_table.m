function [] = print_table(y_in, FMT, no_index, table_header, prnt_range, xlsout, counter_exists, SCREEN, RowSepLine, datefrmat)
% FUNCTION: print table or timetable to screen / or excel
% 
% USAGE: print_table(y_in, FMT, no_index, table_header, prnt_range, xlsout, counter_exists, SCREEN, RowSepLine)
% -----------------------------------------------------------------------------------------
% FMT = '%10.4f';
% FMT = {'%2.6f'};
...
SetDefaultValue(5, 'prnt_range', [])
SetDefaultValue(6, 'xlsout', 0)
% set to zero to force not using the counter check in column 1 in case it creates problems
SetDefaultValue(7, 'counter_exists', []) 
% set to 0 to not print output to screen
SetDefaultValue(8, 'SCREEN', 1)
SetDefaultValue(9, 'RowSepLine', 0)
SetDefaultValue(10,'datefrmat', 'm')
% --------------------------------------------------------------------------------------------------
...
if ~isempty(prnt_range)
  if length(prnt_range) == 1
    y_in = y_in(1:prnt_range,:);
  else
  y_in = y_in(prnt_range,:);
  end
end

if length(datefrmat) == 1
  switch lower(datefrmat)
    case 'd'
      datefrmat = 'dd.mm.yyyy';
    case 'm'
      datefrmat = 'mmm-yyyy';
    case 'q'
      datefrmat = 'yyyy:QQ';
...
% CHECK TYPE OUTPUT --- for tables with cell structure 
tc_types = varfun(@class, y_in, 'OutputFormat', 'cell')';
NoCell = sum(contains(tc_types,'cell'));
if NoCell == 1
  y = y_in{:,2:end};
  RowNames_TT = y_in{:,1};
  ColNames_   = y_in.Properties.VariableNames{2};
else 
% else just use standard array conversion
y   = table2array(y_in);
end % of end of types check
...
if isTT
  ...
  RowNames_ = cellstr(datestr(datenum(y_in.Properties.RowTimes)));
else
...
if isTT 
  oos = print_sample_period(RowNames_TT, 'm',0);
else
...
if SCREEN == 1
  % add date to top in case you want to print TT to screen
  if isTT; TTin.rnames = char(['Date';cellstr(datestr(RowNames_, datefrmat))]); end
  if table_header == 0
    mprint_func(X, TTin, sep_width, RowSepLine )
  else
    if header_empty
      sep(sep_width)
      fprintf(['   ' oos '\n'] );
...
% check/make file names and directory
if (xlsout~=0)
	if ischar(xlsout)
    dirArgs = (strfind(xlsout,'/'));
    if ~isempty(dirArgs)
      set_dir(xlsout(1:max(dirArgs)));
    end
  ...
  if header_empty
    ...
    if isTT
      RowNames_	= datestr(RowNames_, datefrmat);
      xlswrite(xlsout_name, [{'Date'} ColNames_],     'Sheet1','A1'); % colnames/headers
      xlswrite(xlsout_name, prntstrng2xls(RowNames_), 'Sheet1','A2'); % Dates
    else
    ...
    if isTT
      RowNames_	= datestr(RowNames_, datefrmat);
      xlswrite(xlsout_name, [{'Date'} ColNames_],     'Sheet1','A2'); % colnames/headers
      xlswrite(xlsout_name, prntstrng2xls(RowNames_), 'Sheet1','A3'); % Dates
    else
...
% HELPER FUNCTION
% --------------------------------------------------------------------------------------------------
% function mprint_func(y, info, sep_width )
function mprint_func(y, info, sep_width, RowSepLine )
...
% see if the user supplied row names and set rnum, correct mistake if this is the case
if rflag == 1
  rnum = 0;
end
...
% if we have row and column labels adjust variable labels and column heading strings  to match the width of the printing format
if rnum == 1
  dstr = 'Obs#';
end
...
if (rflag == 0 & cflag == 0)
  ...
  for j=1:nsets
    ...
    for k = begr:endr % print row labels and numbers in matrix
      ...
      % UNCOMMENT TO ADDD LINES
      if ~(RowSepLine == 0)
        if any(k == [RowSepLine]); sep(300); end
      end
    end % end of for k
       % fprintf(fid,'\n');
       % fprintf(fid,'\n------------------------------------------------------------\n');
