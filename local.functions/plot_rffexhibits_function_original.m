...
% set to 0 to lead from individual files. I have deleted the individual files from Laptop, they are
% on the Uni Desktop machine, not needed once combined. 
load_combined_files = 1; 
% these have also been already computed, but can be recomputed if needed for different time periods and then stored. 
load_performance_filename = 0; 
...
gy_bechmark_data_file = [gybench_datadir 'gybench-trnwin-' num2str(trnwin) '-stdize-' num2str(stdize), ...
  '-demean-' num2str(demean) '.mat'];
% load GY benchmark in '../Step1_Predictions/tryrff_v2_SeparateSims/gybench-trnwin-12-stdize-1-demean-0.mat'
tic; load(gy_bechmark_data_file);
disp('    GY-benchmark loaded')
toc;

%% load individual iSim Files ... these have aleady been created, 
% Y_B_file_save = ['trnwin-' num2str(trnwin) '-gamma-' num2str(gamma) '-stdize-' num2str(stdize) '-demean-' num2str(demean) '-Y-B'];
Y_B_file_save = ['trnwin-' num2str(trnwin) '-gamma-' num2str(gamma) '-stdize-' num2str(stdize) '-demean-' num2str(demean) '-Yprd_timing'];
filename = strcat(combined_data_save_path, Y_B_file_save, '.mat');

% load_combined_files are in 
% './combined_data/trnwin-12-gamma-2-stdize-1-demean-0-Yprd_timing.mat'
                 % trnwin-12-gamma-2-stdize-1-demean-0-Yprd_timing
tic; 
if isfile(filename)
% if load_combined_files
...
% size(Yprd)
disp('    Combined Data Loaded')
toc

% percentile list
pctlist     = [1 2.5 5 25 50 75 95 97.5 99];
% compute pihat timing weights outside of main loop
pihat       = [squeeze(nanmean(Yprd(:,nP,nL,:),4)) squeeze(nanmean(Yprd(:,1,1,:),4))];
...
% ss = 1;
    ...
    % if load_performance_filename
       ...
       % save the data
        save(performance_filename,'locev','ER','SR', 'vol','IR','IRt','alpha','R2',...
            ...
            'pihat', '-v7.3', '-nocompression');
            % 'Bnrmbar','timing', 'pihat', 'Yprd', '-v7.3', '-nocompression');
    end
    disp('    Performance Data Loaded')
    toc;
    %% PLOTS
    nK = 5; set(groot,'defaultAxesXTickLabelRotationMode','manual') 
    FNS = 16;

    % R2 (average of R2 from 1000 RFF portfolio timing strategies)
    subplot(nK,2,5)
    plot(Plist/trnwin,R2,'linewidth',1.5);line([1,1],[-10,1],'LineStyle','--','color',.5*ones(1,3)); 
    xlabel('$c$','interpreter','latex');
    % set(gcf,'Position',[321 241 512 384]);
    % get(gcf,"Position")
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 

    title('$R^2$','FontWeight','normal','Interpreter','latex')
    ...
    % set(lh,'Position', [.13 .5 .05 .05],'FontSize',12);
    ...
    if gamma == 2
        set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-10,.1])
    elseif gamma == 1 && trnwin == 12
        set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-10,.2])
    elseif gamma == 0.5 && trnwin == 60 && ss <3
        set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-10,.25])
    else
        set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-10,max(max(R2)) + 0.1])
    end
    ytickformat('%.1f')
    if saveon==1
    ...
    %     set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-.1,.1])
    % elseif gamma == 1 && trnwin == 12
    %     set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-.1,.2])
    % else
    %     set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-.1,max(max(R2)) + 0.01])
    % end
    % ytickformat('%.2f'); ylim([-3 0])
    % % yline(0,'LineWidth',2,'HandleVisibility','off','Color','k'); 
    % ytickformat('%.1f')
    % set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    % xline(1,'--','color',.5*ones(1,3));  
    % 
    ...
    xlabel('$c$','interpreter','latex')
    title('Sharpe ratio','FontWeight','normal')
    
    % set(gcf,'Position',[321 241 512 384]);
    set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[min(min(sqrt(12)*SR)) - 0.01,max(max(sqrt(12)*SR)) + 0.01])
    ylim([0 .5])
    % end
    ytickformat('%.2f')
    xline(1,'--','color',.5*ones(1,3));  
    ...
    % close 'all'
    % saveas(gcf,[figdir suffix '-SR-all.eps'],'eps2c');
    
    % ER
    subplot(nK,2,7)
    plot(Plist/trnwin,ER,'linewidth',1.5); 
    xlabel('$c$','interpreter','latex')
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    title('Expected Return','FontWeight','normal')
    % set(gcf,'Position',[321 241 512 384]);
    ...
    set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',ylim_range);
    ytickformat('%.2f')
    ylim([0 0.05])
    xline(1,'--','color',.5*ones(1,3));  
    ...
    if subbeg(ss)==1926 % && subend(ss)==1974
        ...
        if saveon==1
            ...
            if max(Plist/trnwin) == 100
                set(gca,'xtick',[0,5,10,95,100])
                breakxaxis([11 94], 0.015);
            elseif max(Plist/trnwin) == 1000
                set(gca,'xtick',[0:10:50,990,1000])
                breakxaxis([51 989], 0.015);
            elseif max(Plist/trnwin) == 200
                set(gca,'xtick',[0,5,10,195,200])
                breakxaxis([11 194], 0.015);
            end
            % saveas(gcf,[figdir suffix '-ER-brkaxis.eps'],'eps2c');
        end 
    end
    ...
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    % set(gcf,'Position',[321 241 512 384]);
    set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[0,min(5, max(max(vol))+ 0.1)])
    ytickformat('%.2f'); ylim([0 5])
    title('Volatility','FontWeight','normal')
    % line([1,1], ylim_range,'LineStyle','--','color',.5*ones(1,3));
    lh = legend([cellstr(strcat(strcat('$\log_{10}(z)=',num2str(log10(lamlist)')),'$'));{'$c=1$'}], ...
      'interpreter','latex','HandleVisibility','off'); 
    set(lh,'Position', [.18 .47 .13 .09]);
    % [0.184795434226041 0.476517264045688 0.130131353770141 0.0880693942701287]
    % % set(lh,'Position', [.7 .5 ]);
    % % lh.Position(1) = 0.72 - lh.Position(3)/2; breakxaxis_legend
    % % lh.Position(2) = 0.4 - lh.Position(4)/2;
    ...
    % Info ratio
    subplot(nK,2,3)
    plot(Plist/trnwin,sqrt(12)*IR,'linewidth',1.5), xline(1,'--','color',.5*ones(1,3)); 
    xlabel('$c$','interpreter','latex')
    % set(gcf,'Position',[321 241 512 384]);
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    title('Information Ratio','FontWeight','normal')

    if gamma == 2 && trnwin == 12 && ss ==1
    set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0,0.33])
    elseif gamma == 1 && trnwin == 12 && ss == 1
    set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0,0.3])
    elseif gamma == 0.5 && trnwin == 12 && ss<3
    set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0,max(max(sqrt(12)*IR)) + 0.01])
    elseif gamma == 0.5 && trnwin == 12 && ss == 3
    set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[min(min(sqrt(12)*IR))- 0.01,max(max(sqrt(12)*IR)) + 0.01])
    else
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[min(min(sqrt(12)*IR))- 0.01,max(max(sqrt(12)*IR)) + 0.01])
    end
    ytickformat('%.2f')
    ...
    % Alpha
    subplot(nK,2,2)
    plot(Plist/trnwin,alpha,'linewidth',1.5); 
    xlabel('$c$','interpreter','latex')
    % set(gcf,'Position',[321 241 512 384]);
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    title('Alpha','FontWeight','normal')
    
    if gamma == 0.5 && trnwin == 12 && ss == 3
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[min(min(alpha))- 0.01,max(max(alpha)) + 0.01])
    elseif gamma == 0.5 && trnwin == 12 && ss == 1
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0, 0.03])
        % set(gca,'ytick',[0,.01,.02,.03])
        set(gca,'ytick',[0, 0.01, 0.02, 0.03])
    elseif max(max(alpha)) > 0.015 && max(max(alpha)) <=0.025
        set(gca,'ytick',[0, 0.01, 0.02, 0.03])
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0,max(max(alpha)) + 0.002])
    elseif min(min(alpha))> 0 && max(max(alpha)) <=0.015
        % set(gca,'ytick',[0, 0.01])
        set(gca,'ytick',[0, 0.01, 0.02, 0.03])
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0,max(max(alpha)) + 0.001])
    else
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[min(min(alpha))- 0.001,max(max(alpha)) + 0.002])
    end
    ytickformat('%.2f'); ylim([0 .03])
    if saveon==1
    ...
    % Alpha t-stat
    subplot(nK,2,4)
    plot(Plist/trnwin,IRt,'linewidth',1.5), xline(1,'--','color',.5*ones(1,3)); 
    xlabel('$c$','interpreter','latex')
    % set(gcf,'Position',[321 241 512 384]);
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    title('Alpha $t-$statistic','FontWeight','normal','Interpreter','Latex')
    if gamma == 0.5 && trnwin == 12 && ss < 3
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[0,max(max(IRt)) + 0.1]);
    else
        set(gca,'fontname','Times New Roman','fontsize',FNS, 'ylim',[min(min(IRt)) - 0.1,max(max(IRt)) + 0.1]);
    end
    ytickformat('%.2f'); ylim([0 3])
    if saveon==1
    ...
    % Bnorm
    subplot(nK,2,6)
    plot(Plist/trnwin,Bnrmbar,'linewidth',1.5), xline(1,'--','color',.5*ones(1,3)); 
    xlabel('$c$','interpreter','latex')
    % set(gcf,'Position',[321 241 512 384]);
    set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
    title('$||\hat\beta||$','FontWeight','normal','Interpreter','latex')

    set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[0,3])
    ytickformat('%.2f'); ylim([0 3])
    if saveon==1
        % saveas(gcf,[figdir suffix '-Bnorm.eps'],'eps2c');
        % saveas(gcf,[figdir suffix '-all-Figs.eps'],'eps2c');
        if max(Plist/trnwin) == 100
            set(gca,'xtick',[0,5,10,95,100])
            breakxaxis([11 94], 0.015);
        elseif max(Plist/trnwin) == 1000
    ...
    saveas(gcf,[figdir suffix '-all-Figs.eps'],'eps2c');
    print2pdf([figdir suffix '-all-Figs']);
    plot2pdf([figdir suffix '-all-Figs']);
...
% tmp1        = plot(datesdt,[pihat movavg(Yprd_gy(:,6),'simple',6)],'LineWidth',1);
tmp1(3).LineWidth = 2;
tmp1(4).LineWidth = 2;
...
set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-.2,.6])
...
box on; grid on;
set(gca,'LineWidth',1); grid on; set(gca,'GridLineStyle',':'); 
yline(0,'LineWidth',1,'Color',0*ones(1,3),'HandleVisibility','off')
% set(gcf,'Position',[1 1 1200 800]);
% if gamma == 2 && trnwin == 12
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-.3,.7])
% elseif gamma == 1 && trnwin == 12
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-0.6,1.4])
% elseif gamma == 0.5 && trnwin == 60
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-0.5,1])
% elseif gamma == 0.5 && trnwin == 12
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-1,1.5])
% elseif gamma == 2 && trnwin == 60
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-0.1,0.3])
% elseif gamma == 1 && trnwin == 60
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-0.2,0.6])
% elseif gamma == 1 && trnwin == 120
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-0.1,0.4])
% elseif gamma == 2 && trnwin == 120
% set(gca,'fontname','Times New Roman','fontsize',FNS,'ylim',[-0.05,0.2])
...
% Mkt or Buy and Hold
% stats   = regstats(Y(loc)',ones(size(Y(loc)')),1,'tstat');
% out     = [out ; nan sqrt(12)*sharpe(Y(loc)',0) stats.tstat.t(1) -min(Y(loc)') skewness(Y(loc)')];
% out
...
% GW(z=0 )vs mkt
stats_vsMKT   = regstats(tmp1,Y','linear',{'tstat','rsquare','r'}); 
tmp1res_vsMKT = stats_vsMKT.tstat.beta(1) + stats_vsMKT.r;
out = [out ; 100*r2tmp sqrt(12)*sharpe(tmp1,0) stats.tstat.t(1) sqrt(12)*sharpe(tmp1res_vsMKT,0) stats_vsMKT.tstat.t(1) nan(1,2) -min(tmp1) skewness(tmp1)];
...
out     = [out ; 100*r2tmp sqrt(12)*sharpe(tmp1,0) stats.tstat.t(1) sqrt(12)*sharpe(tmp1res_vsMKT,0) stats_vsMKT.tstat.t(1) nan(1,2) -min(tmp1) skewness(tmp1)];
%
% % out     = [out ; nan sqrt(12)*sharpe(tmp1res_vsMKT,0) stats_vsMKT.tstat.t(1) min(tmp1res) skewness(tmp1res)];
% 
% % tmp1    = timing_gy(:,2);
...
% % r2tmp   = 1-nanvar(Y(loc)'-Yprd_gy_tmp1(loc))/nanvar(Y(loc)');
% % stats   = regstats(tmp1,ones(size(Y')),1,'tstat');
% % out     = [out ; r2tmp sqrt(12)*sharpe(tmp1,0) stats.tstat.t(1) min(tmp1) skewness(tmp1)];
% 
...
stats   = regstats(tmp1,ones(size(Y')),1,'tstat');
% RFF(z = 1000) High complexity vs mkt
stats_vsMKT   = regstats(tmp1,Y','linear',{'tstat','rsquare','r'}); 
% % sqrt(12)*stats.tstat.beta(1)/nanstd(stats.r); stats.tstat.t(1);
tmp1res_vsMKT = stats_vsMKT.tstat.beta(1) + stats_vsMKT.r;
% out     = [out ; nan sqrt(12)*sharpe(tmp1res,0) stats.tstat.t(1) min(tmp1res) skewness(tmp1res)];
% out     = [out ; 100*r2tmp sqrt(12)*sharpe(tmp1,0) stats.tstat.t(1) sqrt(12)*sharpe(tmp1res_vsMKT,0) stats_vsMKT.tstat.t(1) nan(1,2) -min(tmp1) skewness(tmp1)];

% RFF(z = 1000) High complexity vs GW(z=1000)
% tmp1    = nanmean(timing(:,end,end,:),4);
stats_vsGWz1000 = regstats(tmp1,timing_gy(:,end),'linear',{'tstat','rsquare','r'}); 
% sqrt(12)*stats.tstat.beta(1)/nanstd(stats.r); stats.tstat.t(1);
tmp1res_vsGWz1000 = stats_vsGWz1000.tstat.beta(1) + stats_vsGWz1000.r;
% out     = [out ; nan sqrt(12)*sharpe(tmp1res,0) stats.tstat.t(1) min(tmp1res) skewness(tmp1res)];
out     = [out ; 100*r2tmp sqrt(12)*sharpe(tmp1,0)              stats.tstat.t(1) ...
                           sqrt(12)*sharpe(tmp1res_vsMKT,0)     stats_vsMKT.tstat.t(1) ...
                           sqrt(12)*sharpe(tmp1res_vsGWz1000,0) stats_vsGWz1000.tstat.t(1) ...
                           -min(tmp1) skewness(tmp1)];
% out
