...
function breakInfo = breakxaxis(varargin)
    ...
    if strcmp(mainLayer,'bottom')
        annotationAxes = CreateAnnotationAxes(mainAxes,mainParent);
    end
    ...
    % leftAxes
    % findobj(leftAxes, 'Type', 'ConstantLine')
    set(leftAxes,'Position', leftPosition, ...
        ...
        'box','off');
    % leftAxes.Label
    % leftAxes.Tag = 'xlinetag'
    % leftAxes.
    % findall(gca, 'Type', 'ConstantLine')
    ...
    % delete(get(leftAxes,'ConstantLine'));
    ...
    %ADJUST TICKELENGTH IF NEEDED
    % tcklength = 0.015;
    % set(leftAxes ,'TickLength',tcklength*ones(1,2))
    % set(rightAxes,'TickLength',tcklength/2*ones(1,2))
    ...
    L1=line('xData',xPoints1,'ydata',yPoints,'Parent',breakAxes,'Color',mainYColor,'LineWidth',mainLineWidth);
    L2=line('xData',xPoints2,'ydata',yPoints,'Parent',breakAxes,'Color',mainYColor,'LineWidth',mainLineWidth);

    % set(breakAxes,'Visible','off');
    set(breakAxes, ...
      'XTick',[], ... 
      'XTickLabel', []);
    % REMOVE THE BREAK AXIS XLABLE EXTRA STRING ENTRY/ OTHER IS COPIED DOWN LATER
    breakAxes.XLabel.String = '';
...
function annotationAxes = CreateAnnotationAxes(mainAxes,mainParent)
